          ...
          <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex items-center gap-4">
              @if (authService.isLoggedIn()) {
                <a routerLink="/app" class="text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">Panel</a>
                <button type="button" (click)="logout()" class="bg-slate-200 text-slate-800 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-300 transition-colors shadow-sm">Wyloguj</button>
              } @else {
      ...
      <main class="flex-1 flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-8 py-20">
        <div class="max-w-3xl space-y-8">
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            @if (authService.isLoggedIn()) {
              <a routerLink="/app" class="bg-blue-600 text-white px-8 py-4 rounded-xl text-lg font-semibold hover:bg-blue-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                Przejdz do panelu
